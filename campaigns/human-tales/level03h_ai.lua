	...
	if ((GetPlayerData(GetThisPlayer(), "TotalNumUnits") > 0) and (GameCycle > 300)) then
		...
		if (GetPlayerData(AiPlayer(), "UnitTypesCount", AiSoldier()) > 20) then
			...
			if (GetNumUnitsAt(AiPlayer(), "unit-human-barracks", {0, 0}, {256, 256}) >= 1) then
				...
				if ((GetNumUnitsAt(6, AiEliteShooter(), {0, 0}, {256, 256}) >= 4) and (GameCycle < 500)) then
					OrderUnits(6, "unit-ranger", 2, 60, 1, 1, 27, 34, "attack")
					OrderUnits(6, "unit-ranger", 2, 61, 1, 1, 27, 35, "attack")
					OrderUnits(6, "unit-ranger", 0, 58, 1, 1, 27, 31, "attack")
					OrderUnits(6, "unit-ranger", 0, 62, 1, 1, 27, 32, "attack")
				end
			end
		end
